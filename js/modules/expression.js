...
const Expression = {
  ...
  renderHistory() {
    ...
    el.innerHTML = this.cards.length ? this.cards.map(row => { const theme = this.themeFor(row); return `<button class="expression-history-card ${row.id === this.selectedCardId ? 'is-active' : ''}" data-expression-card="${this.esc(row.id)}">${theme ? `<span>${theme}</span>` : ''}<strong>${this.esc(row.card?.title || row.title || '未命名练习')}</strong><small>${new Date(row.updated_at || Date.now()).toLocaleDateString('zh-CN')}</small></button>`; }).join('') : '<div class="empty-hint">生成后的练习卡会在这里沉淀成你的选题库。</div>';
  },
};
