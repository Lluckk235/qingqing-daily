...
const Expression = {
  ...
  renderIdeas() {
    ...
    el.innerHTML = this.weeklyIdeas.length ? this.weeklyIdeas.map(idea => {
      ...
      return `<article class="expression-idea ${generated ? 'is-generated' : ''}"><div class="expression-idea-meta"><span class="expression-status-dot ${generated ? 'is-ready' : ''}" aria-label="${generated ? '已有练习卡' : '尚未生成练习卡'}"></span><span class="expression-category">${this.esc(idea.category)}</span></div><h3><button class="expression-idea-title" data-expression-idea="${this.esc(idea.id)}">${this.esc(idea.title)}</button></h3><p>${this.esc(idea.brief || '')}</p><footer><button class="btn-text" data-expression-idea="${this.esc(idea.id)}">${generated ? '查看练习卡 →' : '点击标题开始练习 →'}</button></footer></article>`;
