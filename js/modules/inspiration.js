...
const Inspiration = {
  ...
  renderDetail() {
    ...
    const tabButtons = `<div class="inspiration-tabs"><button class="btn-text ${this.tab === 'overview' ? 'is-selected' : ''}" data-inspiration-action="overview">速览</button><button class="btn-text ${this.tab === 'analysis' ? 'is-selected' : ''}" data-inspiration-action="analysis">原片拆解</button><button class="btn-text ${this.tab === 'script' ? 'is-selected' : ''}" data-inspiration-action="script">灵感文案</button></div>`;
  ...
  scriptHtml(script) {
    ...
    return `<div class="inspiration-sections inspiration-script"><section><h3>核心句</h3><p class="inspiration-lead">${this.esc(script.core)}</p></section><section><h3>灵感文案</h3><p class="inspiration-script-copy">${this.esc(script.script)}</p><button class="btn-secondary" data-inspiration-action="copy">复制灵感文案</button></section><section><h3>逐段画面提示</h3><ol class="inspiration-structure">${beats.map(row => `<li><strong>${this.esc(row.segment)}</strong><span>${this.esc(row.spoken)}</span><small>画面：${this.esc(row.visual)}<br>文字：${this.esc(row.text)}<br>声音：${this.esc(row.sound)}</small></li>`).join('') || '<p class="inspiration-muted">暂未生成</p>'}</ol></section><section><h3>拍摄提醒</h3><ul class="inspiration-bullets">${(script.notes || []).map(note => `<li>${this.esc(note)}</li>`).join('')}</ul></section></div>`;
  },
